...
void main()
{
     ...
     for (uint i=0; i<ivalence; ++i) {
        ...
        opos += f[i];
        oOutput.v.r[i] = (neighbor_p-neighbor_m)/3.0f + (diagonal - diagonal_m)/6.0f;
    }

    opos /= ivalence;
    oOutput.v.position = vec4(opos, 1.0f).xyz;
    oOutput.v.zerothNeighbor = zerothNeighbor;
    ...
    vec3 e;
    oOutput.v.e0 = vec3(0,0,0);
    oOutput.v.e1 = vec3(0,0,0);

    for(uint i=0; i<ivalence; ++i) {
        uint im = (i + ivalence -1) % ivalence;
        e = 0.5f * (f[i] + f[im]);
        oOutput.v.e0 += csf(ivalence-3, 2*i) *e;
        oOutput.v.e1 += csf(ivalence-3, 2*i + 1)*e;
    }
    oOutput.v.e0 *= ef[ivalence - 3];
    oOutput.v.e1 *= ef[ivalence - 3];
    ...
    if (valence < 0) {
        ...
        for (uint x=1; x<ivalence - 1; ++x) {
            ...
            oOutput.v.e1 += alpha * neighbor + beta * diagonal;                         
        }
...
void main()
{
    ...
    uint ip = (i+1)%4;
    uint im = (i+3)%4;
    uint n = uint(abs(iInput[i].v.valence));
    uint ivalence = abs(iInput[i].v.valence);
    int base = GregoryQuadOffsetBase;

    oOutput[ID].v.position = iInput[ID].v.position;
    ...
    prev=uint(prev/256);
    uint np = abs(iInput[ip].v.valence);
    uint nm = abs(iInput[im].v.valence);
    ...
    if (iInput[ip].v.valence < -2) {
        uint j = (np + prev_p - iInput[ip].v.zerothNeighbor) % np;
        Em_ip = iInput[ip].v.position + cos((M_PI*j)/float(np-1))*iInput[ip].v.e0 + sin((M_PI*j)/float(np-1))*iInput[ip].v.e1;
    } else {
        Em_ip = iInput[ip].v.position + iInput[ip].v.e0*csf(np-3,2*prev_p) + iInput[ip].v.e1*csf(np-3,2*prev_p+1);
    }

    uint start_m = uint(texelFetchBuffer(g_QuadOffsetBuffer, int(4*gl_PrimitiveID+base + im)).x)&0x00ff;
    vec3 Ep_im;

    if (iInput[im].v.valence < -2) {
        uint j = (nm + start_m - iInput[im].v.zerothNeighbor) % nm;
        Ep_im = iInput[im].v.position + cos((M_PI*j)/float(nm-1))*iInput[im].v.e0 + sin((M_PI*j)/float(nm-1))*iInput[im].v.e1;
    } else {
        Ep_im = iInput[im].v.position + iInput[im].v.e0*csf(nm-3,2*start_m) + iInput[im].v.e1*csf(nm-3,2*start_m+1);
    }

    if (iInput[i].v.valence < 0) {
        n = (n-1)*2;
    }
    if (iInput[im].v.valence < 0) {
        nm = (nm-1)*2;
    }  
    if (iInput[ip].v.valence < 0) {
        np = (np-1)*2;
    }

    if (iInput[i].v.valence > 2) {
        Ep = iInput[i].v.position + (iInput[i].v.e0*csf(n-3,2*start) + iInput[i].v.e1*csf(n-3,2*start+1));
        Em = iInput[i].v.position + (iInput[i].v.e0*csf(n-3,2*prev) +  iInput[i].v.e1*csf(n-3,2*prev+1)); 

        float s1=3-2*csf(n-3,2)-csf(np-3,2);
        float s2=2*csf(n-3,2);

        Fp = (csf(np-3,2)*iInput[i].v.position + s1*Ep + s2*Em_ip + iInput[i].v.r[start])/3.0f; 
        s1 = 3.0f-2.0f*cos(2.0f*M_PI/n)-cos(2*M_PI/nm);
        Fm = (csf(nm-3,2)*iInput[i].v.position + s1*Em + s2*Ep_im - iInput[i].v.r[prev])/3.0f;

    } else if (iInput[i].v.valence < -2) {
        uint j = (ivalence + start - iInput[i].v.zerothNeighbor) % ivalence;

        Ep = iInput[i].v.position + cos((M_PI*j)/float(ivalence-1))*iInput[i].v.e0 + sin((M_PI*j)/float(ivalence-1))*iInput[i].v.e1;
        j = (ivalence + prev - iInput[i].v.zerothNeighbor) % ivalence;
        Em = iInput[i].v.position + cos((M_PI*j)/float(ivalence-1))*iInput[i].v.e0 + sin((M_PI*j)/float(ivalence-1))*iInput[i].v.e1;

        vec3 Rp = ((-2.0f * iInput[i].v.org - 1.0f * iInput[im].v.org) + (2.0f * iInput[ip].v.org + 1.0f * iInput[(i+2)%4].v.org))/3.0f;
        vec3 Rm = ((-2.0f * iInput[i].v.org - 1.0f * iInput[ip].v.org) + (2.0f * iInput[im].v.org + 1.0f * iInput[(i+2)%4].v.org))/3.0f;
        ...
        if (iInput[im].v.valence < 0) {
            ...
            Fp = Fm = (csf(np-3,2)*iInput[i].v.position + s1*Ep + s2*Em_ip + iInput[i].v.r[start])/3.0f;
        } else if (iInput[ip].v.valence < 0) {
            s1 = 3.0f-2.0f*cos(2.0f*M_PI/n)-cos(2.0f*M_PI/nm);
            Fm = Fp = (csf(nm-3,2)*iInput[i].v.position + s1*Em + s2*Ep_im - iInput[i].v.r[prev])/3.0f;
        }

    } else if (iInput[i].v.valence == -2) {
        Ep = (2.0f * iInput[i].v.org + iInput[ip].v.org)/3.0f;
        Em = (2.0f * iInput[i].v.org + iInput[im].v.org)/3.0f;
        Fp = Fm = (4.0f * iInput[i].v.org + iInput[(i+2)%n].v.org + 2.0f * iInput[ip].v.org + 2.0f * iInput[im].v.org)/9.0f;
    }
    ...
    if (ID == 0) {
        ...
        gl_TessLevelOuter[0] =
            TessAdaptive(iInput[0].v.hullPosition.xyz, iInput[1].v.hullPosition.xyz, patchLevel);
        gl_TessLevelOuter[1] =
            TessAdaptive(iInput[0].v.hullPosition.xyz, iInput[3].v.hullPosition.xyz, patchLevel);
        gl_TessLevelOuter[2] =
            TessAdaptive(iInput[2].v.hullPosition.xyz, iInput[3].v.hullPosition.xyz, patchLevel);
        gl_TessLevelOuter[3] =
            TessAdaptive(iInput[1].v.hullPosition.xyz, iInput[2].v.hullPosition.xyz, patchLevel);
        gl_TessLevelInner[0] =
...
void main()
{
    ...
    p[0] = iInput[0].v.position;
    p[1] = iInput[0].v.Ep;
    p[2] = iInput[0].v.Em;
    p[3] = iInput[0].v.Fp;
    p[4] = iInput[0].v.Fm;

    p[5] = iInput[1].v.position;
    p[6] = iInput[1].v.Ep;
    p[7] = iInput[1].v.Em;
    p[8] = iInput[1].v.Fp;
    p[9] = iInput[1].v.Fm;

    p[10] = iInput[2].v.position;
    p[11] = iInput[2].v.Ep;
    p[12] = iInput[2].v.Em;
    p[13] = iInput[2].v.Fp;
    p[14] = iInput[2].v.Fm;

    p[15] = iInput[3].v.position;
    p[16] = iInput[3].v.Ep;
    p[17] = iInput[3].v.Em;
    p[18] = iInput[3].v.Fp;
    p[19] = iInput[3].v.Fm;
    ...
    oOutput.v.position = ModelViewMatrix * vec4(WorldPos, 1.0f);
    oOutput.v.normal = normal;
    oOutput.v.patchCoord = iInput[0].v.patchCoord;
    oOutput.v.patchCoord.xy = vec2(v, u);
