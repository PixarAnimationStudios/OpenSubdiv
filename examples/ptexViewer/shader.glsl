...
void main()
{
    ...
    oOutput.v.normal = (ModelViewMatrix * vec4(normal, 0)).xyz;
}
...
//    texColor = vec4(pow(texColor.xyz, vec3(0.4545)), 1);
...
#if USE_PTEX_NORMAL
    vec3 normal = perturbNormalFromDisplacement(iInput.v.position.xyz,
                                                iInput.v.normal,
                                                iInput.v.patchCoord);
#else
    vec3 normal = iInput.v.normal;
#endif

    if (overrideColorEnable) {
        texColor = overrideColor;
        vec4 Cf = lighting(texColor, iInput.v.position.xyz, normal);
        outColor = edgeColor(Cf, iInput.v.edgeDistance);
        return;
    }
...
#ifdef USE_PTEX_SPECULAR
    float specular = PTexLookup(iInput.v.patchCoord,
    ...
    vec3 eye = normalize(iInput.v.position.xyz - vec3(0,0,0));
